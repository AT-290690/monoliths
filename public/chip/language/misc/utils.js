...
_mapRemove = (map, key) => { map.delete(key); return map }, _mapSet = (map, key, value) => { map.set(key, value); return map }, _scanLeft = (array, callback) => { for (let i = 0; i < array.length; ++i) callback(array[i], i, array); return array },
_scanRight = (array, callback) => {  for (let i = array.length - 1; i >= 0; --i) callback(array[i], i, array); return array }, _mapLeft = (array, callback, copy = new Brrr()) => { for (let i = 0; i < array.length; ++i) copy.set(i, callback(array.at(i), i, array)); return array.balance() },
_mapRight = (array, callback, copy = new Brrr()) => {  for (let i = array.length - 1; i >= 0; --i) copy.set(i, callback(array.at(i), i, array)); return array.balance() } , _filter = (array, callback) => array.filter(callback) , _reduceLeft = (array, callback, out = []) => array.reduce(callback, out),
_reduceRight = (array, callback, out = []) => array.reduceRight(callback, out), _findLeft = (array, callback) => array.find(callback), _findRight = (array, callback) => array.findLast(callback), _repeat = (n, callback) => { let out; for (let i = 0; i < n; ++i) out = callback(); return out }, 
_every = (array, callback) => array.every(callback), _some = (array, callback) => array.some(callback), _append = (array, value) => array.append(value), _prepemd = (array, value) => array.prepend(value), _head = (array) => array.head(), _tail = (array) => array.tail(), _cut = (array) => array.cut(), 
_chop = (array) => array.chop(), _slice = (array, n1, n2) => array.slice(n1, n2), _length = (array) => array.length, _split = (string, separator) => Brrr.from(string.split(separator)), _join = (arr, separator) => arr.join(separator), _at = (array, index) => array.at(index), _set = (array, index, value) => array.set(index, value), 
_partition = (array, parts) => array.partition(parts), _mSort = (array, callback) => array.mergeSort(callback), _qSort = (array, dir) => array.quickSort(dir), _grp = (array, callback) => array.group(callback), _rot = (array, n, dir) => array.rotate(n, dir), _flatMap = (array, callback) => array.flatten(callback), 
_flat = (array, n) => array.flat(n), call = (x, fn) => fn(x), printout = (...args) => console.log(...args), protolessModule = methods => { const env = Object.create(null); for (const method in methods) env[method] = methods[method]; return env };`
